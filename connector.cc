...
Connector::~Connector() { assert(!channel); }
...
void Connector::resetChannel() { channel.reset(); }
...
void Connector::retry(int32_t sockfd) {
  ...
  if (connect) {
    ...
    loop->runAfter(
        retryDelayMs / 1000.0, false,
        std::bind(&Connector::startInLoop, shared_from_this(), false));
#ifdef _WIN64
    retryDelayMs = (retryDelayMs * 2) < (kMaxRetryDelayMs) ? (retryDelayMs * 2)
                                                           : (kMaxRetryDelayMs);
#else
