...
Epoll::Epoll(EventLoop *loop)
    : events(64), loop(loop), sockfd(::epoll_create1(EPOLL_CLOEXEC)) {
  assert(sockfd >= 0);
}

Epoll::~Epoll() { Socket::close(sockfd); }

void Epoll::epollWait(ChannelList *activeChannels, int32_t msTime) {
  int32_t numEvents =
      ::epoll_wait(sockfd, &*events.begin(), events.size(), msTime);
...
void Epoll::fillActiveChannels(int32_t numEvents,
