...
bool Socket::resolve(std::string_view hostname, struct sockaddr_in6 *out) {
  ...
  char buffer[64 * 1024];
  int32_t ret = ::gethostbyname_r(hostname.data(), &hent, buffer, sizeof buffer,
                                  &he, &herrno);
  if (ret == 0 && he != nullptr) {
...
bool Socket::setTimeOut(int32_t sockfd, const struct timeval tv) {
  ...
  if (::setsockopt(sockfd, SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv,
                   sizeof(tv)) == -1) {
    return false;
  }
  return true;
...
#else
  if (::setsockopt(fd, IPPROTO_TCP, TCP_KEEPALIVE, (const char *)&keepidle,
                   sizeof(keepidle)) < 0) {
    return false;
  }
#endif
...
#ifdef SO_REUSEPORT
  int32_t optval = on ? 1 : 0;
  int32_t ret = ::setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT, &optval,
                             static_cast<socklen_t>(sizeof optval));
  if (ret < 0 && on) {
...
#ifndef _WIN64
  int32_t optval = on ? 1 : 0;
  int32_t opt = ::setsockopt(sockfd, IPPROTO_TCP, TCP_NODELAY, &optval,
                             static_cast<socklen_t>(sizeof optval));
  if (opt < 0) {
    return false;
  }
