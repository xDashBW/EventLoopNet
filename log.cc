...
AppendFile::AppendFile(const std::string &filename)
    : filename(filename), writtenBytes(0) {
#ifdef _WIN64
...
LogFile::LogFile(const std::string &filePath, const std::string &basename,
                 size_t rollSize, bool threadSafe, int32_t interval,
                 int32_t checkEveryN)
    : filePath(filePath),
      basename(basename),
      rollSize(rollSize),
...
void LogFile::appendUnlocked(const char *logline, int32_t len) {
  std::experimental::filesystem::space_info info =
      std::experimental::filesystem::space(filePath.c_str());
  if (info.free <= rollSize * interval) {
    std::cout << ".        Capacity       Free      Available\n"
              << ":   " << info.capacity << "   " << info.free << "   "
              << info.available << '\n';
    return;
  }
...
AsyncLogging::AsyncLogging(std::string filePath, std::string baseName,
                           size_t rollSize, int32_t interval)
    : filePath(filePath),
      baseName(baseName),
      interval(interval),
      running(false),
...
Logger::Impl::Impl(LogLevel level, int32_t savedErrno, const SourceFile &file,
                   int32_t line)
    : stream(),
      level(level),
      line(line),
...
Logger::Logger(SourceFile file, int32_t line) : impl(INFO, 0, file, line) {}
