...
int64_t Timer::getSequence() { return sequence; }

TimeStamp &Timer::getExpiration() { return expiration; }

int64_t Timer::getWhen() { return expiration.getMicroSecondsSinceEpoch(); }

bool Timer::getRepeat() { return repeat; }

void Timer::setSequence(int64_t seq) { sequence = seq; }

double Timer::getInterval() { return interval; }
...
std::string TimeStamp::toFormattedString(bool showMicroseconds) const {
  char buf[32] = {0};
  time_t seconds =
      static_cast<time_t>(microSecondsSinceEpoch / kMicroSecondsPerSecond);
  struct tm tm;
  time_t now = time(0);
  tm = *(localtime(&now));
  if (showMicroseconds) {
    int microseconds =
        static_cast<int>(microSecondsSinceEpoch % kMicroSecondsPerSecond);
...
void TimerQueue::reset(const TimeStamp &now) {
  ...
  for (auto &it : expired) {
    if (it.second->getRepeat() &&
        cancelingTimers.find(it.second->getSequence()) ==
            cancelingTimers.end()) {
